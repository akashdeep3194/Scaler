class Solution:
    ...
    def recfn(self, A, B, C, dp):
        ...
        while k >= 0:
            ...
            for i in range(1,len(dp)):
                if i-A[k] >= 0:
                    dp[i][z] = min(dp[i-A[k]][1-z] + C[k], dp[i][1-z])
                    if dp[i][z]<=B:
                        answer = max(answer,i)
                else:
                    dp[i][z] = dp[i][1-z]
    ...
    def solve(self, A, B, C):
        mx = max(A)
        dp = [[-1 for _ in range(2)]for _ in range(mx*len(A)+1)]
        for i in range(mx*len(A)+1):
            dp[i][-1] = 10**9
        for j in range(2):
            dp[0][j] = 0

        answer = self.recfn(A, C, B, dp=dp)
