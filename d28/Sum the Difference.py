class Solution:
    ...
    # @return an integer
    def modpow(self, a:int, b:int, c:int):
        # return a^b%c
        ans = 1
        if b == 0:
            return 1
    ...
    def solve(self, A):
        ...
        for i,ele in enumerate(A):
            ms = (ms + ((ele)*self.modpow(2,i,c))%c)%c
            ns = (ns + ((ele)*self.modpow(2,n-i-1,c))%c)%c
        if ms-ns<0:
            return ms-ns+c
        else:
            return ms-ns
